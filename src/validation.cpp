...
public:
    ...
    bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, BlockSet* sForkTips) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
const std::string strMessageMagic = "Tidecoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s TDC additional fees, %d delta bytes\n",
...
{
    int halvings = 0;
    int high = 0, interval = consensusParams.nSubsidyHalvingInterval;
    for(int i=0; i<=64; i++){
        high = high + interval;
        if(nHeight >= high){
            halvings++;
...
void ThreadScriptCheck() {
    RenameThread("tidecoin-scriptch");
 ...
 */
bool CChainState::ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock, bool &postponeRelay) {
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock, bool &postponeRelay, BlockSet &sForkTips)
{
    AssertLockNotHeld(cs_main);
    {
        CBlockIndex *pindex = nullptr;
            ...
            try {
                ...
                uint256 hash = block.GetHash();
                {
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                      CValidationState state;
                      if (g_chainstate.AcceptBlock(pblock, state, chainparams, nullptr, true, dbp, nullptr, &sForkTips)) {
                          nLoaded++;
                      }
                      if (state.IsError()) {
                          break;
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus()))
                        {
                            ...
                            if (g_chainstate.AcceptBlock(pblockrecursive, dummy, chainparams, nullptr, true, &it->second, nullptr, &sForkTips))
                            {
                                nLoaded++;
